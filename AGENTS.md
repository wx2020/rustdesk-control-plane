...
- `admin/`：Node.js ≥20 控制面（唯一运行时依赖 `pg`）
  - `server.js`：HTTP API、子进程管理、策略判定、保留清理、Token 轮换、OAuth 路由
  ...
  - `oauth.js`：OAuth 2.0 / OIDC (Authelia) 客户端、PKCE (S256)、State 签名与防篡改校验、UserInfo 抽取与 JIT 用户建档
  ...
  - `oauth.test.js` / `server.test.js`：OAuth PKCE、发现与回调单测与集成测试
  - `postgres.test.js`：需 `TEST_DATABASE_URL`，否则跳过
  - `public/`：中文单页控制台（原生 HTML/CSS/JS，无框架），首屏三轨状态、状态色体系、响应式布局、单点登录 UI 适配
...
- `docs/`：`AUTHENTICATION-OAUTH.md`（Authelia 接入指南与配置范例）、`IMPLEMENTATION-TODO.md`、`OPERATIONS-RUNBOOK.md`、`SE-ARCHITECTURE.md`
...
- `npm test`：**19 通过 / 4 跳过**（4 项 PostgreSQL 集成测试：migration 幂等、20 并发设备唯一约束、20 并发事件幂等+会话投影、JSONB——需 TEST_DATABASE_URL）
